...
void qx_tree_filter::update_table(double sigma_range)
{
	...
	m_table[1]=exp(-double(1 * 0.15)/(QX_DEF_CHAR_MAX*sigma_range));
	m_table[2]=exp(-double(2 * 0.15)/(QX_DEF_CHAR_MAX*sigma_range));

	m_table[QX_DEF_CHAR_MAX]=0;//exp(-double(i)/(QX_DEF_CHAR_MAX*sigma_range));//weight table
}
...
int qx_tree_filter::build_treeDisparity(unsigned char*texture, unsigned char*disparity)
{
	m_mst.mstDisparity(texture, disparity);
	m_mst_parent=m_mst.get_parent();
	m_mst_nr_child=m_mst.get_nr_child();
	m_mst_children=m_mst.get_children();
	m_mst_rank=m_mst.get_rank();
...
int qx_tree_filter::filterDisparity(double*cost,double*cost_backup,unsigned char *src,unsigned char *dst)
{
	int nr_plane = 1;
	memcpy(cost_backup,cost,sizeof(double)*m_h*m_w*nr_plane);
	memcpy(dst,src, m_w * m_h);
	int*node_id=m_node_id;
	int*node_idt=&(node_id[m_nr_pixel-1]);
	for(int i=0;i<m_nr_pixel;i++)
	{
		...
		if(nr_child>0)
		{
			...
			for(int j=0;j<nr_child;j++)
			{
				...
				for(int k=0;k<nr_plane;k++)
				{
					double valNew = (*value_child++)*weight;
					if((dst[id] == 0 || value_sum[k] < valNew) && dst[id_child] != 0)
					{
						value_sum[k] = valNew;
						if(dst[id] == 0 || abs(dst[id] - dst[id_child]) > 1)
						dst[id] = dst[id_child];
					}
				}
			}
	...
	for(int i=1;i<m_nr_pixel;i++)//K_00=f(0,00)[K_0-f(0,00)J_00]+J_00, K_00: new value, J_00: old value, K_0: new value of K_00's parent
	{
		...
		{
			double vc=*value_current++;
			double vcParent = weight * (*value_parent++);
			if((dst[id] == 0 || vcParent > vc) && dst[parent] != 0)
			{
				*value_final++ = vcParent;
				if(dst[id] == 0 || abs(dst[id] - dst[parent]) > 1)
				dst[id] = dst[parent];
			}
			else
				*value_final++ = vc;
