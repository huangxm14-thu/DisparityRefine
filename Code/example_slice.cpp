...
void stereo(char*filename_disparity_map,char*filename_left_image,char*filename_right_image,int max_disparity,bool use_post_processing)
{
	...
	m_nlca.init(h / SLICE_NUM,w,max_disparity,sigma);//initialization

	qx_timer timer;//
	timer.start();

	int offset =  h * slice / SLICE_NUM;
	m_nlca.matching_cost(left + offset,right + offset);//compute matching cost
	timer.time_display("Matching cost computation");
	
	timer.start();
	m_nlca.disparity(disparity + offset,use_post_processing);//compute disparity
