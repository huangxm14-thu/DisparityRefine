...
void qx_nonlocal_cost_aggregation::clean()
{
	...
	qx_freed_4(m_buf_d4); m_buf_d4=NULL;
}
int qx_nonlocal_cost_aggregation::init(int h,int w,int nr_plane,double sigma_range,double max_color_difference,double max_gradient_difference,double weight_on_color)
{
	...
	m_buf_d4=qx_allocd_4(4,m_h,m_w,1);
	m_disparity_vol=m_buf_d4[0];
	m_disparity_vol_temp=m_buf_d4[2];
	...
	m_tf_disparity.init(m_h,m_w,3,m_sigma_range,4);
    return(0);
}
...
int qx_nonlocal_cost_aggregation::FillHoleLR_Simple(unsigned char**disparityL, unsigned char *texture)
{
	int max_disparity = 255;
	unsigned char ***m_buf_u2_=qx_allocu_3(3,m_h,m_w);
	unsigned char **Fill_disparityL=m_buf_u2_[0];
	unsigned char **Fill_disparityR=m_buf_u2_[1];
	...
	for(int y=0;y<m_h;y++) 
	{
		for(int x=0;x<m_w;x++) 
		{
			if(m_mask_occlusion[y][x])
			{
				if(x==0)
					Fill_disparityL[y][x] = max_disparity;
				else
					Fill_disparityL[y][x] = Fill_disparityL[y][x-1];
			}
			else
				Fill_disparityL[y][x] = disparityL[y][x];
		}
	}
	...
	for(int y=0;y<m_h;y++) 
	{
		for(int x=m_w - 1;x>=0;x--) 
		{
			...
			{
				if(x == m_w - 1)
					Fill_disparityR[y][x] = max_disparity;
				else
					Fill_disparityR[y][x] = Fill_disparityR[y][x+1];
			}
			else
				Fill_disparityR[y][x] = disparityL[y][x];
		}
	}
	...
	for(int y=0;y<m_h;y++) 
	{
		for(int x=0;x<m_w;x++)
		{
			if(Fill_disparityL[y][x] < Fill_disparityR[y][x])
				disparityL[y][x] = Fill_disparityL[y][x];
			else
				disparityL[y][x] = Fill_disparityR[y][x];
		}
	}

	qx_freeu_3(m_buf_u2_); 
...
void qx_detect_occlusion_right_left(unsigned char**depth_left,unsigned char**depth_right,int h,int w,int nr_plane)
{
	for(int y=0;y<h;y++) 
	{
		for(int x=0;x<w;x++) 
		{
			int d=depth_right[y][x];
			int xl=x+d;
			if(xl>=0 && xl<w)
			{
				if(d==0||abs(d-depth_left[y][xl])>=1)
				{
					depth_right[y][x]=0;
				}
			}
...
int qx_nonlocal_cost_aggregation::refineFast(unsigned char**disparity,unsigned char**disparityL,unsigned char**disparityR)
{
	int radius=2;

	//left right check
	qx_detect_occlusion_left_right(m_mask_occlusion,disparityL,disparityR,m_h,m_w,m_nr_plane);

	//fill hole
	FillHoleLR_Simple(disparityL, m_left[0][0]);

	//initialize disparity belief
	for(int y=0;y<m_h;y++) for(int x=0;x<m_w;x++) 
	{
		if(m_mask_occlusion[y][x])
			m_disparity_vol[y][x][0]=0.1;
		else
			m_disparity_vol[y][x][0]=1;
	}


	//create hybrid MST
	m_tf_disparity.build_treeDisparity(m_left[0][0], disparityL[0]);

	//belief aggregation on hybrid MST
	m_tf_disparity.update_table(g_nSigmaHybrid * FACTOR_AMPLIFY);
	m_tf_disparity.filter(m_disparity_vol[0][0],m_disparity_vol_temp[0][0],1);


	//belief propagation on MST
	m_tf.update_table(m_sigma_range/2);
	m_tf.filterDisparity(m_disparity_vol[0][0],m_disparity_vol_temp[0][0],disparityL[0], disparity[0]);

	memcpy(m_disparity[0],disparity[0], m_w * m_h);
	ctmf(m_disparity[0],disparity[0],m_w,m_h,m_w,m_w,radius,1,m_h*m_w);
