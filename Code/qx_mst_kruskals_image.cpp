...
inline void qx_mst_compute_edges_per_pixel_disparity(int**edges,unsigned char*distance,unsigned char*image,unsigned char*disparity,int nr_channel,int &nr_edge,int y0,int x0,int yt,int xt,int h,int w)
{
	int id0=edges[nr_edge][0]=qx_mst_yx_2_image_index(y0,x0,h,w);
	int idt=edges[nr_edge][1]=qx_mst_yx_2_image_index(yt,xt,h,w);
	if(nr_channel==1) 
	...
	id0=edges[nr_edge][0]=qx_mst_yx_2_image_index(y0,x0,h,w);
	idt=edges[nr_edge][1]=qx_mst_yx_2_image_index(yt,xt,h,w);
	double distColor = distance[nr_edge];
	double distDepth = abs(disparity[idt]-disparity[id0]);
	double dist = (distColor * g_colorWeight + distDepth * (1 - g_colorWeight)) * FACTOR_AMPLIFY;
	distance[nr_edge] = (dist < QX_DEF_CHAR_MAX) ? dist : QX_DEF_CHAR_MAX;
	if(disparity[id0] == 0 || disparity[idt] == 0)
		distance[nr_edge] = QX_DEF_CHAR_MAX;
	nr_edge++;
}
...
inline int qx_mst_compute_edges_4neighbor_disparity(int**edges,unsigned char*distance,unsigned char*image,unsigned char*disparity,int nr_channel,int h,int w)
{
	int y0,yt,x0,xt;
	int nr_edge=0;
	for(y0=0;y0<h;y0++)
	{
		...
		{
			xt=x0+1;
			qx_mst_compute_edges_per_pixel_disparity(edges,distance,image,disparity,nr_channel,nr_edge,y0,x0,yt,xt,h,w);
		}
	}
	for(int x0=0;x0<w;x0++)
	{
...
inline int qx_mst_compute_edges_8neighbor_disparity(int**edges,unsigned char*distance,unsigned char*image,unsigned char*disparity,int nr_channel,int h,int w)
{
	int y0,yt,x0,xt;
	int nr_edge=qx_mst_compute_edges_4neighbor_disparity(edges,distance,image,disparity,nr_channel,h,w);
	for(y0=0;y0<h-1;y0++)
	{
		yt=y0+1;
		for(int x0=0;x0<w-1;x0++)
		{
			xt=x0+1;
			qx_mst_compute_edges_per_pixel_disparity(edges,distance,image,disparity,nr_channel,nr_edge,y0,x0,yt,xt,h,w);
			qx_mst_compute_edges_per_pixel_disparity(edges,distance,image,disparity,nr_channel,nr_edge,y0,xt,yt,x0,h,w);
		}
	}
	return(nr_edge);
}
...
{
//	nr_neighbor = QX_DEF_MST_KI_8NR_NEIGHBOR;
	clean();
...
int qx_mst_kruskals_image::mstDisparity(unsigned char*image,unsigned char*disparity)
{
	qx_timer timer;
	timer.start();
	init_mst();
	...
	if(m_nr_neighbor==QX_DEF_MST_KI_4NR_NEIGHBOR) qx_mst_compute_edges_4neighbor_disparity(m_edge,m_distance,m_image,disparity, m_nr_channel,m_h,m_w);//find edges
	else qx_mst_compute_edges_8neighbor_disparity(m_edge,m_distance,m_image,disparity, m_nr_channel,m_h,m_w);
	//timer.fps_display();
	timer.start();
	qx_sort_increase_using_histogram(m_id_edge,m_distance,m_nr_edge);//sorting edges in a nondesc
	//timer.fps_display("qx_sort_increase_using_histogram");
